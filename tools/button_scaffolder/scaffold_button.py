...
checks collisions vs cyd-panel.yaml + ~/cortex/main.py, and prints up to
...
CYD_YAML = REPO_ROOT / "cyd-panel.yaml"
...
def check_cyd_collisions(spec: dict[str, Any], cyd_text: str) -> str:
    ...
    if re.search(rf"\bid:\s*{re.escape(btn_id)}\b", cyd_text):
        sys.exit(_err_and_exit(2, f"id collision: 'btn_{bid}' already exists in cyd-panel.yaml"))
    if re.search(rf"\bid:\s*{re.escape(lbl_id)}\b", cyd_text):
        sys.exit(_err_and_exit(2, f"id collision: 'lbl_{bid}' already exists in cyd-panel.yaml"))
    ...
    if not m:
        sys.exit(_err_and_exit(2, f"slot {slot} not found — id '{slot_id}' missing in cyd-panel.yaml"))
...
def main(argv: list[str]) -> int:
    ...
    if is_stateful:
        ...
        print(_framed(
            "BLOCK 1 of 4 — paste into cyd-panel.yaml `globals:` section\n"
    ...
    if is_stateful:
        ...
        print(_framed(
            "BLOCK 2 of 4 — paste into cyd-panel.yaml `interval:` section\n"
    ...
    if spec["page"] == "CTRL":
        slot = spec["slot"]
        insertion = (
            f"INSERTION: in cyd-panel.yaml, search 'id: btn_slot{slot}' on page_main.\n"
            f"           REPLACE the entire `- button:` placeholder block with this widget."
        )
    else:
        insertion = (
            f"INSERTION: in cyd-panel.yaml on page_{spec['page'].lower()}.\n"
    ...
    print(
        "Next steps:\n"
        f"  1. Paste blocks above into cyd-panel.yaml + ~/cortex/main.py\n"
        f"  2. (if Block 4) Cortex: ausfleischen + rebuild + restart\n"
        f"  3. CYD: esphome compile cyd-panel.yaml && esphome upload cyd-panel.yaml --device 192.168.1.240\n"
